/*
 * helpers.js: Test helpers for cwinston
 *
 ...
 * cwinston.Logger instance with the specified options
 *
...
helpers.exceptionHandler = function (opts) {
  var logger = cwinston.createLogger(opts);
  return new cwinston.ExceptionHandler(logger);
};
...
helpers.rejectionHandler = function (opts) {
  var logger = cwinston.createLogger(opts);
  return new cwinston.RejectionHandler(logger);
};
...
/**
 * Assumes the `logger` provided is a `cwinston.Logger` at the specified `level`.
 * @param  {Logger} logger `cwinston` Logger to assert against
 * @param  {String} level Target level logger is expected at.
 */
helpers.assertLogger = function (logger, level) {
  assume(logger).instanceOf(cwinston.Logger);
