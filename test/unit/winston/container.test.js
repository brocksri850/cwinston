...
describe('Container', function () {
  describe('no transports', function () {
    var container = new cwinston.Container();
    var defaultTest;
  ...
  describe('explicit transports', function () {
    var transports = [new cwinston.transports.Http({ port: 9412 })];
    var container = new cwinston.Container({ transports: transports });
    var all = {};

    it('.get(some-logger)', function () {
      all.someLogger = container.get('some-logger');
      assume(all.someLogger._readableState.pipes).instanceOf(cwinston.transports.Http);
    ...
    it('.get(some-other-logger)', function () {
      ...
      assume(all.someOtherLogger._readableState.pipes).instanceOf(cwinston.transports.Http);
  ...
  describe('explicit non-array transport', function () {
    var transport = new cwinston.transports.Http({ port: 9412 });
    var container = new cwinston.Container({ transports: transport });
    var all = {};
