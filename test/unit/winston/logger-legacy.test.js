/*
 * logger-legacy.test.js: Tests for Legacy APIs of cwinston < 3.0.0
 *
 ...
 * are properly handled by a cwinston Logger.
 */
function assumeAcceptsLegacy({ displayName, name, TransportClass }) {
  return function () {
    it(`.add(${name})`, function () {
      ...
      assume(output.stderr.join('')).to.include(`${name} is a legacy cwinston transport. Consider upgrading`);
    });
    ...
    it(`.add(${name}) multiple`, function () {
      ...
      var errorMsg = `${name} is a legacy cwinston transport. Consider upgrading`;
    ...
    it('.remove() [LegacyTransportStream]', function () {
      ...
      const logger = cwinston.createLogger({ transports: transports });
...
describe('Logger (legacy API)', function () {
  ...
  describe(
    'LegacyTransport (inherits from cwinston@2 Transport)',
    assumeAcceptsLegacy({
  ...
  describe(
    'LegacyMixedTransport (inherits from cwinston@3 Transport)',
    assumeAcceptsLegacy({
