...
const format = require('../../../lib/cwinston').format;
...
describe('Logger Instance', function () {
  describe('Configuration', function () {
    it('.configure()', function () {
      let logger = cwinston.createLogger({
    ...
    describe('remove', function () {
      it('should do nothing if transport was not added', function () {
        let transports = [
          ...
          new cwinston.transports.File({filename: path.join(testLogFixturesPath, 'filelog.log')})
        ];

        let logger = cwinston.createLogger({transports: transports})
            .remove(new cwinston.transports.Console());
  ...
  describe('Log Levels', function () {
    it('report unknown levels', function (done) {
      ...
      assume(output.stderr).deep.equals(['[cwinston] Unknown logger level: bar\n']);
      done();
    });
    ...
    describe('Log Levels Enabled', function () {
      it('default levels', function () {
        let logger = cwinston.createLogger({
          level: 'verbose',
          levels: cwinston.config.npm.levels,
          transports: [new cwinston.transports.Console()]
        });
      ...
      it('default levels, transport override', function () {
        let transport = new cwinston.transports.Console();
  ...
  describe('Transport Events', function () {
    it(`'finish' event awaits transports to emit 'finish'`, function (done) {
      ...
      const finished = [];
      const logger = cwinston.createLogger({transports});
    ...
    it('error', (done) => {
      const consoleTransport = new cwinston.transports.Console();
  ...
  describe('Backwards Compatability', function () {
    describe('cwinston V2 Log', function () {
      ...
      it('.log(level, formatStr, ...splat)', function (done) {
        const format = cwinston.format.combine(
            cwinston.format.splat(),
            cwinston.format.printf(info => `${info.level}: ${info.message}`)
        );
      ...
      it('.log(level, formatStr, ...splat, meta)', function (done) {
        const format = cwinston.format.combine(
            cwinston.format.splat(),
            cwinston.format.printf(info => `${info.level}: ${info.message} ${JSON.stringify({thisIsMeta: info.thisIsMeta})}`)
        );
