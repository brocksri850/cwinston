...
describe('File (maxsize)', function () {
  ...
  it('should create multiple files correctly when passed more than the maxsize', function (done) {
    ...
    const maxsizeTransport = new cwinston.transports.File({
      level: 'info',
    ...
    // Otherwise the maxsizeTransport._dest is undefined. See https://github.com/brocksri850/cwinston/issues/1174
    //
  ...
  describe('With lazy option enabled', () => {
    it('should not create extra file', function (done) {
      const fillWith = ['a', 'b', 'c', 'd', 'e'];
      const lazyTransport = new cwinston.transports.File({
        format: cwinston.format.printf(info => info.message),
        filename: path.join(testLogFixturesPath, 'testmaxsize.log'),
        maxsize: 3072,
        lazy: true
