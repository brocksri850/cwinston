...
const defaultLevels = cwinston.config.npm.levels;
const transports = {
  defaults: new cwinston.transports.Console(),
  noStderr: new cwinston.transports.Console({ stderrLevels: [] }),
  stderrLevels: new cwinston.transports.Console({
    stderrLevels: ['info', 'error']
  }),
  consoleWarnLevels: new cwinston.transports.Console({
    consoleWarnLevels: ['warn', 'debug']
  }),
  eol: new cwinston.transports.Console({ eol: 'X' }),
  syslog: new cwinston.transports.Console({
    levels: cwinston.config.syslog.levels
  }),
  customLevelStderr: new cwinston.transports.Console({
    levels: {
      alpha: 0,
      beta: 1,
      gamma: 2,
...
describe('Console transport', function () {
  ...
  describe('throws an appropriate error when', function () {
    it("if stderrLevels is set, but not an Array { stderrLevels: 'Not an Array' }", function () {
      assume(function () {
        let throwing = new cwinston.transports.Console({
...
// vows.describe('cwinston/transports/console').addBatch({
...
//       cwinston.config.npm.levels, defaultTransport, ['debug', 'error'], stdMocks
//     )
//   }
// }).addBatch({
...
//       cwinston.config.npm.levels, debugStdoutTransport, ['error'], stdMocks
//     )
//   }
// }).addBatch({
...
//       cwinston.config.npm.levels, stderrLevelsTransport, ['info', 'warn'], stdMocks
//     )
//   }
// }).addBatch({
...
//       cwinston.config.npm.levels, noStderrTransport, [], stdMocks
//     )
//   }
// }).addBatch({
